...
fn main() {
    ...
    let marbles = processed[2].parse::<i32>().unwrap() + 1;
...
fn play_game(players: &i32, marbles: &i32) -> HashMap<i32, i32> {
    ...
    for marble in 1..*marbles {
        ...
        if is_scoring_marble(&marble) {
            let removal_index = bind_index(&index, -7, &game_state);
...
//            println!("Player {} scores {} points!", curr_player, score);
            ...
            let insert_index = bind_index(&index, 2, &game_state);
...
fn bind_index<T>(index: &usize, shift: i32, vec: &Vec<T>) -> usize {
    let len = vec.len() as i32;
    if len == 0 {
        return 0;
    }

    let mut i = *index as i32 + shift;
    while i < 0 {
        i += len;
    }
    while i >= len {
