...
def main():
    ...
    root = parse_node(numbers, 0)

    nodes = build_node_list(root)

    print(get_value(nodes[0]))


def build_node_list(root):
    node_list = [root]
    for child in root["children"]:
        node_list.extend(build_node_list(child))
    return node_list
...
def get_value(node):
    ...
    val = 0
    for m in node["meta"]:
        if m - 1 < node["child_count"]:
            val += get_value(node["children"][m - 1])
    return val
