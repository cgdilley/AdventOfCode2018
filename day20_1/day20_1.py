...
def main():
    ...
    locations = find_rooms_at_minimum_distance(base_map, 1000)

    print("Identified %d rooms that are at least 1000 doors away." % len(locations))
...
def find_rooms_at_minimum_distance(base_map: dict, min_dist: int) -> list:
    """
    Finds all of the rooms in the given base map that are at a distance away from the starting room
    greater than or equal to the given minimum distance.

    :param base_map: The base map
    :param min_dist: The minimum distance for a room to be considered
    :return: A list of all room locations that are at least the minimum distance away from the starting
    point
    """
    # ========================================================
    # ========================================================
    # EVERYTHING COMMENTED OUT HERE IS STUPID AND USELESS

    # over_min = set()
    # known = {(0, 0): 0}
    # bounds = get_bounds(base_map)
    #
    # # spiral inwards... makes sense to me as the most efficient path to process things, but
    # # have no proof
    #
    # loc = (bounds["left"] - 1, bounds["top"])
    # direction = 0
    # spiral = "ESWN"
    #
    # while bounds["left"] < bounds["right"] and bounds["top"] < bounds["bottom"]:
    #     new_loc = move(loc, spiral[direction])
    #     if not in_bounds(bounds, new_loc):
    #         direction = (direction + 1) % 4
    #         if spiral[direction] == "S":
    #             bounds["top"] += 1
    #         elif spiral[direction] == "N":
    #             bounds["bottom"] -= 1
    #         elif spiral[direction] == "W":
    #             bounds["right"] -= 1
    #         elif spiral[direction] == "E":
    #             bounds["left"] += 1
    #         continue
    #     loc = new_loc
    ...
    #         continue
    #
    #     if manhattan((0, 0), loc) >= min_dist:
    #         over_min.add(loc)
    #     else:
    #         path, g = get_path(base_map, (0, 0), loc, known)
    #         for l in path:
    #             known[l] = g[l]
    #             if known[l] >= min_dist:
    #                 over_min.add(l)
    ...
    # I tried different approaches to the pathfinding, including trying to cache intermediate
    # calculations of paths to be reused by future pathfinding.  But turns out all of that
    # was stupid, because all I had to do was make it try form a path to a point that doesn't exist,
    # in which case it traverses all reachable squares (exhausting all options) and finds the shortest
    # path to each of them in the process.  A*/Dijkstra's is cool.

    path, g = get_path(base_map, (0, 0), (1000, 1000))

    over_min = list(filter(lambda loc: g[loc] >= min_dist, g.keys()))
...
def get_path(base_map, start, end):
    ...
    f = {loc: manhattan(loc, end) + g for loc, g in g.items()}
    ...
    while len(available) != 0:
        ...
        if found_end is not None and found_end < f[current]:
            return build_path(base_map, g, start, end), g
