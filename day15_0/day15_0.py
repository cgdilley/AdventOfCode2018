...
def execute_round(battle_map: dict) -> bool:
    ...
    for unit_coord in move_order:
        ...
        if unit_coord not in battle_map["_units"]:
            continue
        ...
        execute_attack(battle_map, unit_coord, 3)
...
def move_unit(battle_map: dict, unit_coord: tuple) -> tuple:
    ...
    attack_paths = {square: get_path(battle_map, unit_coord, square) for square in attack_squares}
    ...
    # Get the first move of the path (path[0] == unit_coord)
    best_move = path[1]
...
def execute_attack(battle_map: dict, unit_coord: tuple, damage: int) -> None:
    """
...
def get_path(battle_map, start, end):
    ...
    while len(available) != 0:
        ...
        # If the coordinate we're looking at is the end coordinate, then we've found the path.
        # Reconstruct it by working backwards through the came_from dictionary
        if current == end:
...
def build_path(came_from: dict, start: tuple, end: tuple) -> list:
    ...
    :param came_from: A dictionary that maps coordinates to the coordinate the brings them closest to the start
