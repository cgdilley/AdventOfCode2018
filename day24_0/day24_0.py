...
def main():
    ...
    state = conduct_battle(immune, infect)
...
def conduct_battle(immune, infect) -> dict:
    """
    ...
    while len(immune) > 0 and len(infect) > 0:
        ...
        perform_round(battle_state)

    return battle_state


def perform_round(battle_state: dict) -> None:
    """
    ...
    :return: None
    """
    targeting = select_targets(battle_state)
    perform_attacks(targeting)
    clean_up_dead(battle_state)


def select_targets(battle_state: dict) -> dict:
    """
    ...
    # Sort all units by effective power, breaking ties with initiative
    all_units = immune + infect
    all_units = sorted(all_units, key=lambda g: g.effective_power() * 100 + g.initiative, reverse=True)

    # Iterate through all units in the sorted order
    for unit in all_units:
        # Calculate the effective power of this unit
        power = unit.effective_power()
        max_damage = 0
        target = None
        ...
        for t in options:
            ...
            if damage > max_damage:
                ...
                max_power = target.effective_power()
                t_power = t.effective_power()
...
class Group:
    ...
    def attack(self, target) -> None:
        ...
        target.take_damage(self.effective_power(), self.atk_type)
